...
/** datetime-local value `offsetMs` from now, in the input's expected format. */
function defaultLocal(offsetMs: number): string {
...
export function CreateBountyForm({ onCreated }: { onCreated?: (bountyId: bigint) => void }) {
  ...
  const [submissionDeadline, setSubmissionDeadline] = useState(defaultLocal(60 * 60 * 1000));
  const [revealDeadline, setRevealDeadline] = useState(defaultLocal(2 * 60 * 60 * 1000));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Ritual block.timestamp is in MILLISECONDS — pass ms directly.
  ...
  return (
    <Card>
      <CardHeader
        ...
        subtitle="Fund a reward, set a submission + reveal deadline. Answers stay hidden until reveal."
      />
      <CardBody>
        ...
        <form onSubmit={handleSubmit} className="mt-3 space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <Field label="Submission deadline" hint="Commits accepted until here.">
              <Input
                type="datetime-local"
          ...
          {createdId !== null && (
            <Notice tone="green">
              ...
              <span className="font-mono font-semibold">#{createdId.toString()}</span>. Loaded below.
            </Notice>
          )}
        </form>
      </CardBody>
